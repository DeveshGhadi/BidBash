import React from "react";
import { Link } from "react-router";

export const CTA = () => {
  return (
    <section className="relative overflow-hidden py-20 sm:py-24">
      <div className="absolute inset-0 bg-gradient-to-br from-gray-500 to-black" />
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-50" />

      <div className="relative max-w-3xl mx-auto px-4 text-center">
        <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">
          Ready to Start Bidding?
        </h2>
        <p className="text-lg text-gray-200 mb-8 max-w-xl mx-auto">
          Join our community today and discover amazing deals or turn your items
          into cash. Getting started takes seconds.
        </p>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link
            to="/signup"
            className="inline-flex items-center justify-center bg-white text-gray-700 font-semibold px-8 py-3.5 rounded-xl hover:bg-black transition-all shadow-sm"
          >
            Create Free Account
          </Link>
          <Link
            to="/auction"
            className="inline-flex items-center justify-center border border-white/30 text-white font-semibold px-8 py-3.5 rounded-xl hover:bg-white/10 transition-all"
          >
            Explore Auctions
          </Link>
        </div>
      </div>
    </section>
  );
};
